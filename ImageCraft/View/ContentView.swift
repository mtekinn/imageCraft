...
struct ContentView: View {
    ...
    var body: some View {
        VStack {
            ...
            if viewModel.isCameraViewShowing {
                CameraView(handlePickedImage: { image in
                    viewModel.image = image
                }, sourceType: .camera)
                    .onDisappear { viewModel.isCameraViewShowing = false }
            }

            if viewModel.isPhotoLibraryViewShowing {
                CameraView(handlePickedImage: { image in
                    viewModel.image = image
                }, sourceType: .photoLibrary)
                    .onDisappear { viewModel.isPhotoLibraryViewShowing = false }
            }
            ...
            if viewModel.image != nil {
                ...
                .sheet(isPresented: $viewModel.isCameraViewShowing) {
                            CameraView(handlePickedImage: { image in
                                viewModel.image = image
                                viewModel.isCameraViewShowing = false
                            }, sourceType: .camera)
                        }
                        .sheet(isPresented: $viewModel.isPhotoLibraryViewShowing) {
                            CameraView(handlePickedImage: { image in
                                viewModel.image = image
                                viewModel.isPhotoLibraryViewShowing = false
                            }, sourceType: .photoLibrary)
                        }
            }
        }
